// A series of files that work together to establish vertices and edges of a graph as well as connect them to one another, and allow the graph to be searched and a shortest path
// made between any two vertices. Time complexity of O(E*Log(V)) due to usage of adjacency list.
// Christian Lovetere | U46489387 | Sec 003
// Evan Inge          | U64219670 | Sec 001
// Twyman Lamb        | U10726017 | Sec 001
...
void Graph::path_start(std::string startLabel){
    //loop through vertices in the graph
...
void Graph::traverse_path(std::string endLabel, std::vector<std::string>& path){
    for (std::deque<Vertex*>::iterator vertex_it = vertices.begin(); vertex_it != vertices.end();) {
		if (endLabel == (*vertex_it)->vertex) { //Keep looping until the vertex iterator is at the end vertex
			...
			(*vertex_it)->shortest_path.push_back(endLabel); //Put the destination at the end of the path
			path = (*vertex_it)->shortest_path; //copy the end destinations path vector into the path vector passed into this function
		}
		else{}
		++vertex_it;
	}
